---
name: todo-workflow
description: TODO/백로그를 만들거나 상태를 바꿀 때, 또는 무거운 항목을 spec/plan으로 올릴지 판단할 때
---

# TODO 관리 워크플로

`todo` 파라미터 디렉터리의 md 파일(SSOT) + GitHub Issues(미러) + Projects 보드로 to-do/백로그를 관리하는 규칙. **번호는 GitHub 이슈 번호를 그대로 쓴다**(3자리 zero-pad: 이슈 `#7` → `#007`). 한 항목에 번호가 두 개 존재하지 않게 하려는 것이다.
에이전트는 TODO 작업을 수행하는 같은 흐름에서 **md → Issue → Project** 순서로 세 곳을 함께 갱신한다.
프로젝트마다 달라지는 값(`issues`·`board`·`todo`)은 맨 아래 "필요한 프로젝트 파라미터"에서 읽는다. 아래 예시의 `docs/to-do/`는 `todo`의 기본값이고, `<owner/repo>`는 `issues`의 값이다.

## 원칙

- **SSOT = repo 안의 md 파일** (`todo` 디렉터리). GitHub Issues는 칸반/Projects용 **미러**일 뿐이다.
- **1 주제 = 1 파일 = 1 이슈.** 관련된 것끼리 묶고, 서로 `참고` 링크로 연결한다.
- **번호 = GitHub 이슈 번호**(`todo-numbering: issue`, 기본값). 직접 배정하지 않는다. 이슈를 먼저 만들어 GitHub가 준 번호를 3자리 zero-pad해서 쓴다(이슈 `#7` → `#007`). 이슈 번호는 PR과 시퀀스를 공유하므로 **미리 예측하지 않는다** — `todo` 디렉터리의 마지막 번호 +1로 찍으면 경쟁 상태로 어긋난다.
...
- **`todo-numbering: local`은 기존 저장소의 잔존 예외다.** 그 저장소는 to-do 번호를 자체 배정하고 이슈 번호와 다르며, 이유가 `decisions`에 있다. 두 번호를 md의 `이슈:` 줄로 잇고, 어느 쪽 번호를 말하는지 항상 밝힌다. **신규 저장소에는 `local`을 쓰지 않는다** — 한 항목에 번호가 둘이면 대화에서 어느 번호인지 매번 되묻게 된다(2026-09-19 확정).
- **2026-08-19 이전에 만든 항목은 번호가 이슈 번호와 어긋날 수 있다.** 소급해서 다시 번호를 매기지 않는다(파일명·이슈 본문·상호 링크가 전부 깨진다). 기존 항목은 그대로 두고 신규 항목부터 일치시킨다.
- **사용자에게 보고할 때 번호만 쓰지 않는다.** to-do를 언급하는 답변은 맨 위에 `#007 : <제목>` 한 줄을 먼저 적고 본문으로 들어간다. 번호만 적힌 보고는 받는 사람이 파일이나 이슈를 열어야 무슨 일인지 안다. 여러 건이면 한 줄씩 나열한다.
- **md 상태 갱신은 작업 브랜치가 아니라 `docs-branch`(기본 `main`)에 직접 커밋한다.** 워크트리/브랜치에서 작업 중이어도 마찬가지다 — Issue와 Project는 이미 브랜치 밖에 있는데 md만 병합을 기다리면 세 곳이 어긋난다. 따라서 **작업 브랜치에서는 `todo` 디렉터리를 건드리지 않는다**(충돌도 같이 사라진다) → [작업 인계](../handoff/SKILL.md)의 "문서는 `docs-branch`로".
- 최상위 `todo` 디렉터리엔 **남은 일만** 둔다. 완료되면 그 아래 `done/`으로 이동한다(날짜는 파일명이 아니라 파일 안에).
...
- Issue repo와 Project owner/number/title은 `issues`·`board` 파라미터에 실제 값으로 적는다. 에이전트가 Project를 이름으로 추측하게 두지 않는다.
...
`<todo>/<NNN>-<주제>.md`(`NNN` = 이슈 번호). 아래 골격이 정본이고, 복사해 쓸 실파일은 dev-conventions `project/todo-file.template.md`다. 템플릿 파일 자체는 `todo` 디렉터리에 두지 않는다(최상위엔 남은 일만).
...
`<owner/repo>`(= `issues` 파라미터)만 바꿔 붙여넣는다.
...
`gh`가 Project를 못 읽으면 먼저 권한을 받는다: `gh auth refresh -s project` → `gh project list --owner <project-owner>`.
...
- **진행 중 맥락은 to-do가 아니라 handoff에** 둔다. to-do는 *무엇을·왜·됐나*의 안정적 SSOT이므로 세션 단위의 휘발성 상태(어디까지 했나·뭘 시도했다 실패했나)를 섞지 않는다 → [작업 인계](../handoff/SKILL.md).
- **개발 격리:** 워크트리는 to-do가 아니라 **병합 단위(plan)** 에 붙인다(에픽 to-do는 우산, 워크트리는 그 밑 plan 단위) → [에이전트 작업 규칙](../agent-workflow/SKILL.md)의 "작업 격리".

## 필요한 프로젝트 파라미터

- `issues` — `github:<owner>/<repo>`. 번호를 받아오는 곳이자 미러.
- `board` — `github-project:<project-owner>/<project-number>` + Project title. 에이전트가 Project를 이름으로 추측하게 두지 않으려고 값으로 박는다.
- `todo` — md SSOT 디렉터리(기본 `docs/to-do/`, 완료분은 그 아래 `done/`).
- `todo-numbering` — `issue`(기본) 또는 `local`. 위 번호 규칙이 이 값으로 갈린다. `local`은 기존 저장소 전용이다.
- `docs-branch` — to-do md를 직접 커밋할 브랜치(기본 `main`) → [작업 인계](../handoff/SKILL.md).
- **완료 기준**은 프로젝트마다 다르므로 `AGENTS.md`에 한 줄로 적는다(예: "`dev` 머지 + `origin/dev` push까지").
- 골격 파일은 dev-conventions `project/todo-file.template.md`다(위 "to-do md 파일 템플릿" 절과 같은 내용).
