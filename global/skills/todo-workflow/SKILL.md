...
- **번호 = GitHub 이슈 번호.** 직접 배정하지 않는다. 이슈를 먼저 만들어 GitHub가 준 번호를 3자리 zero-pad해서 쓴다(이슈 `#7` → `#007`). 이슈 번호는 PR과 시퀀스를 공유하므로 **미리 예측하지 않는다** — `todo` 디렉터리의 마지막 번호 +1로 찍으면 경쟁 상태로 어긋난다.
