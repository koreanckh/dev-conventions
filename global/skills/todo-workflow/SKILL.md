...
- **`todo-numbering: local`인 저장소는 예외다.** 그 저장소는 to-do 번호를 자체 배정하고 이슈 번호와 다르며, 이유가 `decisions`에 있다. 두 번호를 md의 `이슈:` 줄로 잇고, 어느 쪽 번호를 말하는지 항상 밝힌다.
...
- `todo-numbering` — `issue`(기본) 또는 `local`. 위 번호 규칙이 이 값으로 갈린다.
