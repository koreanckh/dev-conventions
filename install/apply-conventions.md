...
4. **1회 셋업** — 적용한 규칙 문서의 "이 규칙 적용하기"에 셋업 명령이 있으면(예: todo-workflow의 gh 라벨 생성) 실행 여부를 물어본 뒤 실행한다. `<owner/repo>` 같은 값은 대상 repo의 git remote에서 채운다.
5. **리포트** — 복사/병합/provider 진입점/셋업 결과를 요약한다.
