...
Q3. (프로젝트) 무엇인가?
    ...
    ├─ 진행 중인 작업의 상태                           → handoff · todo (`docs-branch` 직행 · 병합 시 삭제)
...
- handoff·to-do 상태·결정 기록은 워크트리/브랜치에서 작업하더라도 `docs-branch`(기본 main)에 직접 커밋한다. 브랜치엔 코드와 그 코드가 병합돼야 참이 되는 문서만 둔다.
- to-do를 언급하는 답변은 번호만 쓰지 말고 맨 위에 `#NNN : <제목>` 한 줄을 적는다.
...
| 진행 상태 | `docs/handoff/<브랜치>.md`, todo | 작업 브랜치가 아니라 `docs-branch`(기본 main)에 직접 커밋. 병합 시 삭제 |
...
- **단계 5 완료.** 복사본을 가진 활성 저장소 3개 전부 전환 — nowhere·bus·mulzipsa. 프로젝트 내 전역 규칙 복사본 13 → **0**. `/apply-conventions`는 세 번의 실제 변환 순서대로 재작성했다.
  - 세 저장소 **모두** to-do 번호를 자체 배정하고 있었다(이슈 번호와 불일치). 전역 기본값 `todo-numbering: issue`를 쓰는 저장소가 **하나도 없다** — 기본값을 뒤집을지는 열린 질문으로 남긴다.
  - 복사본에는 예외 없이 프로젝트가 덧붙인 내용이 섞여 있었다(Node 버전 함정·커밋 type·브랜치 규칙). **지우기 전 드리프트 검사**가 이 절차의 핵심 단계이고, 커맨드에 그렇게 박았다.
  - 범위 밖으로 남은 복사본: `blog`(3) · `law`(6). 활성 여부 확인 후 처리.
- **단계 6 부분 검증.** 새로 clone한 저장소에서 가짜 HOME으로 `bootstrap.sh`를 돌려 전 과정을 확인했다 — 심링크·hook 경로가 clone 위치를 가리키고(자기 위치 감지), 재실행 0건, 설정 디렉터리가 없는 에이전트는 건너뛴다. **남은 미검증분은 "다른 기계"뿐이다**(다른 OS·경로·에이전트 버전, 사람 손이 몇 번 가는지).
- **단계 7 착수.** `handoff` skill의 "병합할 때"에 승격 트리거를 넣었다(교훈이 사라지기 직전 지점에서 한 번 묻는다, 전역은 직접 고치지 않는다). 첫 심사 결과는 `inbox/2026-09-18-nowhere-promotion-review.md`:
...
### 문서 브랜치와 to-do 번호 (2026-09-19)

- **문서는 작업 브랜치가 아니라 `docs-branch`(기본 `main`)에 직접 커밋한다.** handoff·to-do 상태·결정 기록은 브랜치가 병합돼야 참이 되는 문서가 아니라 **지금 참인 문서**다. 브랜치에 갇히면 정작 읽어야 할 다른 세션·다른 PC가 fetch하기 전엔 못 보고, 워크트리를 여러 개 굴리면 브랜치마다 만든 handoff가 main에서 충돌한다. 판정 기준은 **"병합 전에도 참인가"** — 참이면 `docs-branch`, 그 브랜치의 코드가 병합돼야 참이 되는 문서(README·API 문서)는 코드와 같은 커밋.
  - **뒤집은 규칙:** "handoff는 코드와 같은 커밋으로 push된다". 그게 막던 실패(*작업물 없이 handoff만 올라감*)는 **순서**로 대체했다 — 브랜치 push → handoff 커밋, 헤더의 sha는 이미 push된 것이어야 한다.
  - **새 파라미터 `docs-branch`**(기본 `main`, 통합 브랜치가 `dev`면 `dev`). main이 보호돼 직접 push가 막히는 저장소만 `branch`로 선언하고 이유를 `decisions`에 남긴다.
- **`todo-numbering` 기본값은 `issue`로 확정한다**(아래 "남은 일"의 열린 질문 종결). 한 항목에 번호가 둘이면 대화에서 어느 번호를 말하는지 매번 되묻게 된다. 기존 `local` 저장소는 **소급 재번호를 하지 않지만**(파일명·이슈 본문·상호 링크가 전부 깨진다) 신규 저장소는 `local`을 쓰지 않는다.
- **to-do를 언급하는 보고는 `#NNN : <제목>`으로 시작한다.** 번호만 적힌 보고는 받는 사람이 파일이나 이슈를 열어야 무슨 일인지 안다.
...
- ~~열린 질문: `todo-numbering`의 기본값.~~ → **종결(2026-09-19): `issue` 유지**(위 절). 기존 `local` 저장소 3개는 그대로 두고 신규 저장소부터 일치시킨다. 남은 일은 그 3개를 언제 어떻게 정렬할지(또는 영구 예외로 둘지)뿐이다.
